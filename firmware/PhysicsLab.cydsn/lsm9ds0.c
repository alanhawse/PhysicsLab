...
#define LSM9DS0STATUS_REG_M		    0x07
...
void LSM9DS0readRegStart(uint8 slave, uint8 reg, int count);
void LSM9DS0readReg(uint8 slave,uint8 reg, uint8 result[], int count);
void LSM9DS0readRegEnd(uint8 result[],int count);
uint16 LSM9DS0WhoAmI();
...
int LSM9DS0test()
{
    ...
    SPI_SpiSetActiveSlaveSelect(LSM9DS0SlaveAccel);
    ...
    SPI_SpiSetActiveSlaveSelect(LSM9DS0SlaveGyro);
...
void LSM9DS0writeReg(uint8 slave, uint8 reg, uint8 val)
{
    uint8 cmd[2];
    cmd[0] = reg;
    cmd[1] = val;
    ...
    SPI_SpiUartClearTxBuffer();
    SPI_SpiUartClearRxBuffer();
    SPI_SpiUartPutArray(cmd,2);
    spiWait();
}

void LSM9DS0readReg(uint8 slave,uint8 reg, uint8 result[], int count)
{
    LSM9DS0readRegStart(slave,reg,count);
    spiWait();
    LSM9DS0readRegEnd(result,count);
}
...
void LSM9DS0enableReadings()
{
	...
	*/
	LSM9DS0writeReg(LSM9DS0SlaveGyro,LSM9DS0_CTRL_REG2_G, 0x00); // Normal mode, high cutoff frequency
	...
	LSM9DS0writeReg(LSM9DS0SlaveGyro,LSM9DS0_CTRL_REG3_G, 0x88); 
	...
	SIM - SPI serial interface mode select
		0=4 wire, 1=3 wire */
	LSM9DS0writeReg(LSM9DS0SlaveGyro,LSM9DS0_CTRL_REG4_G, 0x00); // Set scale to 245 dps
	...
	LSM9DS0writeReg(LSM9DS0SlaveGyro,LSM9DS0_CTRL_REG5_G, 0x00);
	...
	LSM9DS0writeReg(LSM9DS0SlaveMag,LSM9DS0_CTRL_REG6_XM, 0x00); // Mag scale to +/- 2GS
	...
	LSM9DS0writeReg(LSM9DS0SlaveMag,LSM9DS0_CTRL_REG7_XM, 0x00); // Continuous conversion mode
	
	/* CTRL_REG4_XM is used to set interrupt generators on INT2_XM
	Bits (7-0): P2_TAP P2_INT1 P2_INT2 P2_INTM P2_DRDYA P2_DRDYM P2_Overrun P2_WTM
	*/
	LSM9DS0writeReg(LSM9DS0SlaveMag,LSM9DS0_CTRL_REG4_XM, 0x04); // Magnetometer data ready on INT2_XM (0x08)
	...
	MIEN - Enable interrupt generation for magnetic data
		0=disable, 1=enable) */
	LSM9DS0writeReg(LSM9DS0SlaveMag,LSM9DS0_INT_CTRL_REG_M, 0x09); // Enable interrupts for mag, active-low, push-pull
   
}
    


typedef enum LSMMode {
    LSMNone,
    LSMAccel,
    LSMMag,
    LSMGyro
...
int LSM9DS0Process()
{
    ...
    static LSMMode mode = LSMNone;
     
    if(SPI_SpiIsBusBusy() || LSM9DS0isBroken())
        return 0;
    ...
    switch(mode)
    {
        case LSMNone:
            mode = LSMAccel;
            LSM9DS0readRegStart(LSM9DS0SlaveAccel,LSM9DS0_OUT_X_L_A,6);

        break;
        
        case LSMAccel:
            // read the result
            LSM9DS0readRegEnd((uint8 *)&LSM9DS0Accel,6);
           

            
            mode = LSMMag;
            LSM9DS0readRegStart(LSM9DS0SlaveMag,LSM9DS0_OUT_X_L_M,6);
        ...
        case LSMMag: 
            // read the result
            LSM9DS0readRegEnd((uint8 *)&LSM9DS0Mag,6);
        

            mode = LSMGyro;
            LSM9DS0readRegStart(LSM9DS0SlaveGyro,LSM9DS0_OUT_X_L_G,6);
            
        break;
        
        case LSMGyro:
             LSM9DS0readRegEnd((uint8 *)&LSM9DS0Gyro,6);
...
uint16 LSM9DS0WhoAmI()
{
    ...
    uint8 result[8];
    LSM9DS0readReg(LSM9DS0SlaveAccel,LSM9DS0_WHO_AM_I_XM,result,1);
    
    rval = result[0] << 8;
    LSM9DS0readReg(LSM9DS0SlaveGyro,LSM9DS0_WHO_AM_I_G,result,1);
